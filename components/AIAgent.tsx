      ...
      <div className="p-4 border-b border-zinc-800 bg-zinc-900/80 backdrop-blur-md flex items-center justify-between z-10">
        <div className="flex items-center gap-3">
          <div className="relative">
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-red-600 to-purple-600 flex items-center justify-center shadow-lg shadow-red-600/20">
          ...
          <div>
            <h3 className="text-sm font-orbitron font-black text-white tracking-tight italic">AGENT <span className="text-red-600">ZERO</span></h3>
            ...
            <div className="flex flex-wrap gap-2 justify-center">
              {starterPrompts.map((prompt, i) => (
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg bg-zinc-800 border border-zinc-700 text-[10px] font-bold text-zinc-400 hover:text-white hover:border-red-600/50 transition-all"
                >
                  {prompt}
                </button>
              ))}
          ...
          return (
            <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[85%] rounded-2xl p-3.5 space-y-3 ${
                msg.role === 'user' 
                ? 'bg-red-700 text-white font-medium shadow-lg' 
                ...
                {msg.role === 'model' && suggestions.length > 0 && (
                  <div className="flex flex-col gap-2 pt-2 border-t border-zinc-700/50">
                    <p className="text-[8px] font-black text-red-600 uppercase tracking-widest">Recommendations:</p>
                    {suggestions.map((s, i) => (
                      <button
                        key={i}
                        onClick={() => onSelectSuggestion(s)}
                        className="flex items-center justify-between gap-3 px-3 py-2 bg-zinc-950/50 hover:bg-red-600 text-white hover:text-white rounded-xl border border-zinc-700 hover:border-red-500 transition-all group/chip"
                      >
      ...
      <form onSubmit={handleSubmit} className="p-4 bg-zinc-900 border-t border-zinc-800">
        <div className="relative flex items-center gap-2">
          <input 
            ...
            className="flex-1 bg-zinc-950 border border-zinc-800 rounded-xl py-3 pl-4 pr-12 text-xs text-white focus:ring-1 focus:ring-red-600/50 outline-none transition-all placeholder:text-zinc-700"
          />
          <button 
            type="submit"
            disabled={!input.trim() || isLoading}
            className="absolute right-2 top-1.5 p-1.5 text-red-600 disabled:text-zinc-800"
          >
